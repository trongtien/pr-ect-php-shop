...
<table class="table table-striped table-advance table-hover">
    ...
    <th></th>
    <?php
    $sql_select_donhang = mysqli_query($conn,"SELECT * FROM donhang inner join member on donhang.userid = member.userid where donhang.status = 0 ");
    $i=0;
    ...
    <td>
        <a  href="danhsachdonhang.php?id=<?php echo $row_donhang['iddonhang'] ?>"
            class="<?php echo $row_donhang['status']==0 ? 'btn btn-danger':'btn btn-success'?>"
            >
            <?php  echo $row_donhang['status']==0 ? 'Đang chờ xử lý' : 'Đã xác nhận'?>
        </a>
    </td>
    <th>
    <div class="btn-group">
